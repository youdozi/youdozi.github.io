...
spring batch 5는 jdk 17이 최소 사양인 spring framework 6 기반으로 한다.
...
oracle과 sql-server인 경우는 sequence 관련 업데이트가 있다.

그외 모든 플랫폼은 아래와 같은 스키마 변경 사항이 있다.

BATCH_JOB_EXECUTION

JOB_CONFIGURATION_LOCATION 컬럼은 사용되지 않는다.
...
batch parameter로 사용되던 string, date, long, double형 대신 <T> 제레릭으로 받을수 있게 변경되었다.

그리하여 각 해당 컬럼들은 삭제되고 PARAMETER_NAME, PARAMETER_TYPE, PARAMETER_VALUE 컬럼이 새로 생성되었다.
...
spring batch 5로 migration하는 경우 위 ALTER 쿼리 실행시 크리티컬한 버그가 있다.

ALTER TABLE BATCH_JOB_EXECUTION_PARAMS CHANGE COLUMN TYPE_CD PARAMETER_TYPE VARCHAR(100);

위 쿼리가 문제인데 spring batch 4일 경우 Long 타입은 TYPE_CD에 LONG 으로 저장되어 있다.

하지만 spring batch 5일 경우  PARAMETER_TYPE 에 java.lang.Long 으로 저장된다.

spring batch 5에서 신규로 만들어진 job이면 문제없지만 이전 버전부터 사용하고 있던 job을 실행하면 오류를 뿜뿜하고 나올 것이다.

수정할 방안은 아래와 같다.

string, date, long, double 의 각각 패키지명까지 포함하여 update 처리

테이블 삭제 후 재생성(not recommend)

spring-cloud-data-flow 2.11.x 버전 적용

각각의 장단점이 있으니 신중하게 고려해서 선택하는것이 좋다.

BATCH_STEP_EXECUTION

CREATE_TIME 컬럼이 신규로 생성되었으며 START_TIME 컬럼에 NOT_NULL 조건이 삭제된다.
...
spring batch 4에서는 아래와 같이 BatchConfigurer을 직접 구현해야 했다.

spring batch 5에서는 BatchConfigurer가 제거되었고 DefaultBatchConfiguration를 상속하여 커스터마이징만 하면 된다.
...
또한 @EnableBatchProcessing 어노테이션방식으로 dataSourceRef, transactionManagerRef 등 다양한 옵션을 넣을 수 있다.
...
spring batch 4까지는 @EnableBatchProcessing 를 통해 트랜잭션 매니저가 spring context에 노출되었다.

하지만 무조건적인 노출은 사용자 정의 트랜잭션 매니저 적용하는데 문제가 발생하였다.

관련 이슈 →

이 부분을 수정하기 위해 tasklet에서도 명시적으로 transactionManager를 구성하도록 하였다.
...
JobBuilderFactory/StepBuilderFactory가 deprecated되었으며 spring context에서도 삭제되었다.

메서드는 아직 삭제되지 않았지만 intellij에서 warning 메시지를 노출할 것이다.

아래와 같이 name, jobRepository를 parameter로 하여 생성하도록 한다.
...
번외 → spring-cloud-data-flow 2.11.x
